...
test.describe("agents", () => {
  test.afterEach(async ({ request }) => {
    await patchAgent(request, "agt-saqr", { paused: false });
  });

  test("Saqr detail shows traces tab and the pause switch flips state via PATCH", async ({ page, request }) => {
    await patchAgent(request, "agt-saqr", { paused: false });

    await page.goto("/agents");
    await page.getByRole("link", { name: /Saqr/ }).first().click();
    await expect(page).toHaveURL(/\/agents\/agt-saqr$/);
    await expect(page.getByRole("heading", { name: /Saqr/ })).toBeVisible();

    // Saqr is the orchestrator: she coordinates but never runs tools herself,
    // so her traces tab renders with the count her API reports (usually 0).
    const { traces } = (await (await request.get("/api/agents/agt-saqr")).json()) as { traces: unknown[] };
    ...
    const patched = page.waitForResponse(patchOf("agt-saqr"));
    await pause.click();
    ...
    await expect(page.getByText(/Saqr paused/)).toBeVisible();

    const unpatched = page.waitForResponse(patchOf("agt-saqr"));
    await pause.click();
    ...
    await expect(page.getByText(/Saqr back on duty/)).toBeVisible();
  });

  test("a tool-running agent lists its traces", async ({ page }) => {
    await page.goto("/agents/agt-hisn");
  ...
  test("detail log is pre-filled from persisted history", async ({ page, request }) => {
    const { events } = (await (await request.get("/api/agents/agt-saqr")).json()) as { events: { id: string; summary?: string }[] };
    expect(events.length).toBeGreaterThan(0);
    expect(events.length).toBeLessThanOrEqual(200);

    await page.goto("/agents/agt-saqr");
