...
import {
  ChatBubble,
  ChatBubbleMessage,
} from "@/components/vendor/shadcn-chat/chat-bubble";
...
function deliverySuffix(d: NonNullable<Message["delivery"]>[number]) {
  if (d.status === "sent")
    return `· sent via ${CHANNEL_LABEL[d.channel] ?? d.channel}`;
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        ...
        {a.subtitle && (
          <span className="block truncate opacity-70">{a.subtitle}</span>
        )}
      </span>
    </Link>
  );
...
export function PhoneConversation({
  ...
  return (
    <div className={cn("flex h-full flex-col bg-bg-0 text-text-1", className)}>
      ...
      <div
        className={cn(
          "flex items-center gap-2 border-b border-white/[0.06] bg-bg-1/90 px-3 backdrop-blur-xl",
          compact ? "pt-3 pb-2" : "pt-12 pb-2",
        )}
      >
        {onBack ? (
          <button
      ...
      <div className="min-h-0 flex-1">
        <ChatMessageList smooth className="gap-0 px-3 py-3">
          {list.map((m, i) => {
            const prev = list[i - 1];
            const newDay =
              !prev || dayLabel(prev.sentAt) !== dayLabel(m.sentAt);
            const gap =
              !prev ||
              ...
              <React.Fragment key={m.id}>
                ...
                >
                  <HoverCard>
                      ...
                      >
                        {m.kind === "alert" && m.severity && (
                          <span
                            ...
                            style={{ background: SEVERITY_HEX[m.severity] }}
                          />
                        )}
                        ...
                        {m.attachments?.map((a) => (
                          <Attachment key={`${a.type}-${a.refId}`} a={a} />
                        ))}
                        {m.tapback && (
                          <span
                            className={cn(
                    ...
                    <HoverCardContent
                      side="top"
                      className="w-auto rounded-full border-white/10 bg-bg-3 px-2 py-1"
                    >
                ...
                {!fromAgent && last && (
                  <span className="mono-data mt-0.5 self-end pr-1 text-[10px] text-text-3">
                    {m.readAt ? "Read" : m.deliveredAt ? "Delivered" : "Sent"}
                  </span>
                )}
                {fromAgent && m.delivery?.length ? (
                  <span className="mono-data mt-0.5 self-start pl-1 text-[10px] text-text-3">
                    {timeShort(m.sentAt)}{" "}
                    {deliverySuffix(m.delivery[m.delivery.length - 1])}
                  </span>
                ) : null}
              </React.Fragment>
            );
          ...
          {typing && (
            <ChatBubble variant="received" className="max-w-[60%]">
              <ChatBubbleMessage
                variant="received"
                className="bubble-system flex items-center gap-2 px-3 py-2"
              >
          ...
          {list.length === 0 && (
            <p className="mt-10 text-center text-[12px] text-text-3">
              No texts yet. Say hi — try{" "}
              <span className="mono-data text-text-2">status</span> or{" "}
              <span className="mono-data text-text-2">help</span>.
            </p>
          )}
        </ChatMessageList>
      </div>
      ...
      >
        ...
        <Button
          ...
          className="size-8 rounded-full bg-lime text-carbon hover:bg-lime/85"
          aria-label="Send"
        >
