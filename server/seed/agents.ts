...
const AGENTS: AgentSpec[] = [
  {
    ...
    callsign: "the falcon",
    role: "orchestrator",
    mandate: "Triage every signal, assign the garrison, keep the operator informed. Owns the operator thread and approval requests.",
    description: "First to every signal. Reads the telemetry feed, correlates it into threats, decides who takes it, and texts the operator when it matters.",
    trust: 5,
  ...
  },
  {
    ...
    description: "Quiet until something real shows up. Then the fastest hands in the garrison — isolates hosts, blocks egress, cordons clusters.",
    trust: 5,
  ...
  },
  {
    ...
    description: "Works the fleet. Runs conformance checks round-robin, fixes drift, patches services, and moves workloads when a host goes bad.",
    trust: 4,
