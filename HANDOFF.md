...
- **Checks**: `npx tsc --noEmit` clean · `npm run lint` 0 errors / 0 warnings · `npm test` 77/77 (blind boundary clean, lint clean) · `npm run build` green (37 API routes incl. `/api/messages/inbound` + `/api/settings/delivery/test`), all pages prerender.
