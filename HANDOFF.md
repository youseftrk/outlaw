...
- **Checks**: `npx tsc --noEmit` clean · `npm test` 103/103 (blind boundary clean) · `npm run build` green (42 API routes incl. `/api/messages/inbound`, `/api/settings/delivery/test`, `/api/settings/ssh/test`, `PATCH /api/fleet/servers/[id]`), all pages prerender.
