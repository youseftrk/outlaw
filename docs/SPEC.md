...
| GET `/settings` · PATCH `/settings` · POST `/settings/llm/test` · POST `/settings/delivery/test` | `Settings` (secrets redacted to `apiKeySet` / `secretSet` / `twilioAuthTokenSet`) · `{llm?:{provider,baseUrl,model,apiKey?,enabled}, operator?, sim?, delivery?:{channel,url,twilio,filter,secret?,twilioAuthToken?}}` · test result · delivery test result (§8.1) |
