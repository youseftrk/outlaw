...
| GET `/settings` · PATCH `/settings` · POST `/settings/llm/test` · POST `/settings/ssh/test` | `Settings` · `{llm?:{provider,baseUrl,model,apiKey?,enabled}, ssh?:{user,port,hostKeyPolicy,sudo,timeoutMs,privateKey?,bastion?,hostMap?,orchestratorUrl?,forgetKnownHosts?}, operator?, sim?}` · test result · `{serverId}` → `echo qalaa-ok` result (502 on failure) |
