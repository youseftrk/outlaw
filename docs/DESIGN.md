...
└──────────────────────────────────┴────────────────┘
```
Copy: title "Command center" (display), eyebrow "Frontier Hub · 28 servers · 6 agents riding".

### `/agents` + `/agents/[id]`
Grid 3 cols of agent cards: `BotAvatar` 96 px (type per agent: Cassidy `star`, Sundance `triangle`, Doc `hexagon`, Belle `flower`, Ringo `square`, Calamity `ghost`; state: acting→`working`, idle→`sleeping`, else `default`; `color` lime for orchestrator, cerulean others), name (display 28 px), callsign + role eyebrow, mandate, status `Status` chip, autonomy `Badge`, tools as small mono `Badge`s, activity sparkline, metrics row (mono). Detail: header (avatar + `ThinkingOrb` if investigating), `Tabs`: Live log (`Terminal` streaming action lines), Traces (rows → `Sheet` trace view), Messages, Servers. Controls: autonomy `ToggleGroup` (observe · recommend · act-with-approval · autonomous), pause `Switch`.
